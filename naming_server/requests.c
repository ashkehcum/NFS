...
void file_requests_to_storage_server(request req, int client_id)
{
    ...
    if(storage_server_id == -1)
    {
        ...
        logMessage(CLIENT_FLAG, client_id, *req, PATH_NOT_FOUND);
        return;
    }
    if(req->request_type == CREATE_FILE)
    {
        ...
        logMessage(CLIENT_FLAG, client_id, *req, PATH_FOUND);
        // ss respose left
    }
    ...
    else
    {
        ...
        if(res == -1)
        {
            ...
            res->Port_No = -1;
            send(client_id, res, sizeof(st_response), 0);
            logMessage(CLIENT_FLAG, client_id, *req, COPY_TO_PATH_INVALID);
            return;
        }
